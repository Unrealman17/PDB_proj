    ...
    for token in tokens:
        if found_select:
            if isinstance(token, sqlparse.sql.IdentifierList):
                return [
                    col.value.split(
                        "--")[0].strip().split(" ")[-1].strip("`").rpartition('.')[-1]
...
def rmdir(path, dbutils=None):
    ...
    if dbutils:
        path = dbr_prepare_path(path)
        def delete_funct(arg): return delete_mounted_dir(
            dirname=arg, dbutils=dbutils)
    else:
...
class Table:
    ...
    def __init__(self,
        ...
        self._spark_context = spark_session
        self.format = format
        table_exist = spark_session._jsparkSession.catalog().tableExists('default', name)
    ...
    def head(self, n=5):
        self.spark_session.sql(f'SELECT * FROM {self.name} LIMIT {n}').show()

    def drop(self):
        if self.spark_session:
            cmd = f'DROP TABLE IF EXISTS {self.name};'
            print(cmd)
            self.spark_session.sql(cmd).collect()
            self.exist = False
    ...
    def spark_session(self, spark: SparkSession):
...
class ServiceTable(Table):
    ...
    def create(self):
        if self.dml:
            if self.spark_session:
                print(self.dml)
                self.spark_session.sql(self.dml)
                self.exist = True
                return
...
class DTable(Table):
    ...
    def create_table_as(self, query: str):
        if self.exist:
            self.drop()
        cmd = f'CREATE '
        ...
        cmd += f'AS\n{query};'
        print(cmd)
        self.spark_session.sql(cmd)

    def insert_overwrite(self, query: str):
        if not self.columns:
...
class BTable(DTable):
    ...
    def build_dataframe(self):
        if not self.dataframe:
            self.dataframe = self._spark_context.createDataFrame(
                self.data, schema=self.schema)
        else:
            new_df = self._spark_context.createDataFrame(
                self.data, schema=self.schema)
    ...
    def write(self):
        ...
        if self.data or (self.dataframe and self.dataframe.count() != 0):
            ...
            if self.exist:
                cmd = self.insert_overwrite(f'SELECT * FROM tmp_{table_name}')
            else:
                cmd = self.create_table_as(f'SELECT * FROM tmp_{table_name}')
...
class STable(DTable):
    ...
    def create_or_replace(self):
        if self.exist:
            cmd = self.insert_overwrite(self.query)
        else:
            cmd = self.create_table_as(self.query)


class Tables_config:
    # reads "tables" folder and "config.json"
    def __init__(self, spark_session: SparkSession, dbutils=None, config=None) -> None:
        self.config = config or CONFIG
        table_describe_path = self.config["table_describe_path"]
        self.service = []
        self.dbutils = dbutils
        service_path = os.path.join(table_describe_path, 'service')
            ...
            if os.path.isfile(file_path):
                if file_name.endswith('.sql'):
                    ...
                    table = ServiceTable(name=file_name[:-len('.sql')],
                                         dml=dml,
                                         external_location=self.config['external_table_path'],
                                         spark_session=spark_session
                                         )
        ...
        self.silver = []
        silver_path = os.path.join(table_describe_path, 'silver_query')
        for file_name in os.listdir(silver_path):
            ...
            if os.path.isfile(file_path):
                if file_name.endswith('.sql'):
                    with open(file_path) as f:
                        query = f.read()
                    # file_name: silver_chem_comp.sql
                    table_name = file_name[:-len('.sql')]
                    # table_name: silver_chem_comp
                    # category: _chem_comp
                    category = file_name[len('silver'):]
                    table = STable(name=table_name,
                                   query=query,
                                   external_location=self.config['external_table_path'],
                                   category=category,
                                   spark_session=spark_session
    ...
    def init_bronze_schema(self, file_name: str = None, block=None):
        ...
        if file_name:
            ...
            block = cif.read_file(file_name).sole_block()
...
def remove(spark_session: SparkSession, config: dict, dbutils=None) -> Tables_config:
    ...
    external_table_path = config['external_table_path']
    rmdir(external_table_path, dbutils=dbutils)
...
@task
def install(spark_session: SparkSession, config: dict, tables_config: Tables_config = None):
    ...
    if not tables_config:
        tables_config = Tables_config(
            spark_session=spark_session, table_describe_path=config['table_describe_path'])
...
@task
def reinstall(spark_session: SparkSession, config: dict, dbutils=None):
    tables_config = remove(spark_session=spark_session,
                           config=config, dbutils=dbutils)
    install(spark_session=spark_session,
            config=config, tables_config=tables_config)
